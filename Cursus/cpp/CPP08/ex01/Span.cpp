...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/09/20 20:08:29 by dbohoyo-          #+#    #+#             */
/*   Updated: 2026/09/24 11:29:27 by dbohoyo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	std::vector<int>::difference_type count = std::distance(begin, end);
	if (count < 0 || this->_array.size() + static_cast<size_t>(count)
		> static_cast<size_t>(this->_size))
		throw Span::VectorIsFull();
...
size_t Span::shortestSpan()
{
	...
	{
		size_t diff = static_cast<size_t>(static_cast<long long>(copy[i])
			- static_cast<long long>(copy[i - 1]));
		if (diff < shortest)
			shortest = diff;
	}
	return (shortest);
...
size_t Span::longestSpan()
{
	...
	size_t diff = static_cast<size_t>(static_cast<long long>(*std::max_element(copy.begin(), copy.end()))
		- static_cast<long long>(*std::min_element(copy.begin(), copy.end())));
	return (diff);
}
...
{
	return(RED "The array not have enough elements!" RESET);
}